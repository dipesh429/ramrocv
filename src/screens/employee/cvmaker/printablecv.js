...
import {getcvid, getdata} from '../../../actions/employee';

class Printable extends Component{

	componentDidMount(){
		if(this.props.cvid){
			this.props.getcv(this.props.cvid);
		}
	...
	render(){
		return (
			<div style={this.props.style} ref={this.props.refcallback}>
						    ...
						    <ul>
								{
									this.props.data.reference.map((d,k) => (
										<li key={k}>
											...
											<h5>Designation: {d.designation}</h5>
											<h5>Phone: {d.phone} </h5>
											<h5>Email: {d.email} </h5>
										</li>
									))
